  ...
  <div>
    ...
    <header class="topbar">
      ...
      <div class="topbar-actions">
        <!-- red close (X) button when the cart is open -->
        <button
          v-if="showCart"
          class="close-cart"
...
export default {
  ...
  computed: {
    ...
    grandTotal: function () {
      var total = 0;
      for (var i = 0; i < this.cart.length; i++) {
        total = total + (this.cart[i].price * this.cart[i].qty);
      }
      return total;
    }
  },
  ...
  methods: {
    ...
    next: function () {
      if (this.currentBanner < this.banners.length - 1) this.currentBanner++;
      else this.currentBanner = 0;
    },
    prev: function () {
      if (this.currentBanner > 0) this.currentBanner--;
      else this.currentBanner = this.banners.length - 1;
    },
    ...
    book: function (lesson) {
      ...
      var existing = -1;
      for (var i = 0; i < this.cart.length; i++) {
        if (this.cart[i].id == lesson.id) existing = i;
      }
      if (existing != -1) this.cart[existing].qty++;
    ...
    checkout: function () {
      ...
      if (!nameValid || !phoneValid) {
        alert("Please enter a valid Name (letters) and Phone (numbers).");
        return;
      }
